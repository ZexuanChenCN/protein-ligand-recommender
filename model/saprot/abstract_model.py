...
class AbstractModel(pl.LightningModule):
    def __init__(self,
        ...
        """
        抽象模型基类，用于定义模型训练的基本流程和接口

        Args:
            lr_scheduler_kwargs (dict, optional): 学习率调度器参数，例如：
                {"init_lr": 1e-5, "max_lr": 1e-4, "final_lr": 1e-6}
            optimizer_kwargs (dict, optional): 优化器参数，例如：
                {"lr": 1e-5, "weight_decay": 0.01}
            save_path (str, optional): 模型保存路径，例如："./checkpoints/model.pt"
            from_checkpoint (str, optional): 从检查点加载模型的路径
            load_prev_scheduler (bool, optional): 是否加载之前的学习率调度器状态
            save_weights_only (bool, optional): 是否仅保存模型权重而不保存优化器状态

        Example:
            >>> model = AbstractModel(
            ...     lr_scheduler_kwargs={"init_lr": 1e-5},
            ...     optimizer_kwargs={"lr": 1e-5, "weight_decay": 0.01},
